package com.nbucciarelli.betplugin.commands;
...
public class BetCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        ...
        if(command.getName().equalsIgnoreCase("bet")){
            ...
            if(randInt > 55){
                Bukkit.broadcastMessage(ChatColor.LIGHT_PURPLE + p.getName() + ChatColor.WHITE+ " bet " + ChatColor.AQUA
                        + quantity + " " + aestheticName + ChatColor.WHITE +"and, won the double!");
                p.getInventory().getItemInMainHand().setAmount(quantity * 2);
            } else{
                Bukkit.broadcastMessage(ChatColor.LIGHT_PURPLE + p.getName() + ChatColor.WHITE+ " bet " + ChatColor.AQUA
                        + quantity + " " + aestheticName + ChatColor.WHITE +"and lost everything away...");
